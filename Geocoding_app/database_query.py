...
connection = "mysql://{username}:{password}@/{db_name}?charset=utf8&unix_socket=/cloudsql/{instance}".format(username=c.user,
                            password=c.password, db_name=c.db, instance=c.instance_name)
...
def fetchMetadata_admin(conn, tableName):
...
def DeleteEntry(conn, tablename, colName, value):
    query = 'DELETE FROM {0} WHERE {1} = "{2}";'.format(tablename, 
                                                        colName, value)
    trans = conn.begin()
    try:
        conn.execute(query)
